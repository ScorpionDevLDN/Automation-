...
function onOpen() {
  ...
  ui.createMenu('Mail Merge')
      .addItem('Send Emails to ALL Recipients', 'sendEmails')
      .addItem('Send Email to SELECTED Recipient', 'sendSelectedEmail')
      .addToUi();
}
...
function sendSelectedEmail(subjectLine, sheet=SpreadsheetApp.getActiveSheet()){
  ...
  const recipientEmail = activeCell.getDisplayValue();
  const emailSentRange =getCellRangeByColumnName(sheet,'Email Sent',activeCell.getRow());
  const firstName = getCellRangeByColumnName(sheet,'First name',activeCell.getRow()).getDisplayValue();
  const description = getCellRangeByColumnName(sheet,'Description',activeCell.getRow()).getDisplayValue();
  const dueDate = getCellRangeByColumnName(sheet,'Due date',activeCell.getRow()).getDisplayValue();
  var out = "";
  var data = {'Recipient':recipientEmail,'First name':firstName,'Description':description,'Due date':dueDate,};
  if ( emailSentRange.getDisplayValue() ==''){
    try {
       
       const msgObj = fillInTemplateFromObject_(emailTemplate.message, data);
      ...
      GmailApp.sendEmail(recipientEmail, msgObj.subject, msgObj.text, {
        // bcc: 'a.bbc@email.com',
        // cc: 'a.cc@email.com',
          from: 'recruiter.support@freelancer.com',
          name: 'Recruiter Team Support',
